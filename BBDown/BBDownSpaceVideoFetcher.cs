...
namespace BBDown
{
    class BBDownSpaceVideoFetcher : IFetcher
    {
        public BBDownVInfo Fetch(string id)
        {
            ...
            string userName = GetValidFileName(JsonDocument.Parse(GetWebSource(userInfoApi)).RootElement.GetProperty("data").GetProperty("name").GetString());
            ...
            var pages = infoJson.RootElement.GetProperty("data").GetProperty("list").GetProperty("vlist");
            foreach (var page in pages.EnumerateArray())
            {
                urls.Add($"https://www.bilibili.com/video/av{page.GetProperty("aid").GetString()}");
            }
